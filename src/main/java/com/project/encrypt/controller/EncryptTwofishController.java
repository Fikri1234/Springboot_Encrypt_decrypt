...
@RestController
public class EncryptTwofishController {
	
	@PostMapping("/twofish")
	public ResponseEntity<?> encryptTwofish(@RequestBody EncryptRequest request) {
	...
	private String encrypt(String input, String secret, int bitNum) throws Exception {
		...
		byte[] encryptedMessage = EncryptionBouncyCastelUtil.encrypt(EncryptionValueConstant.ENCRYPTION_SCHEME_TWOFISH,
	...
	private String decrypt(String input, String secret, int bitNum) throws Exception {
		...
		byte[] decryptedMessage = EncryptionBouncyCastelUtil.decrypt(EncryptionValueConstant.ENCRYPTION_SCHEME_TWOFISH,
